...
- [ ] Task 3: Build closed-session chronological backtests with time-gap uncertainty, leakage tests, and abstention evaluation. The deterministic binary-target harness/baselines, continuous envelope audit, aggregate-only read-only audit runner, and user-approved broad unseen-task prior pass synthetic tests. A continuous ordinary-log learner now records one direct work point per closed-session exercise/basis; its first point is neutral and later same-basis points update cautiously. The current export has three closed weighted-evidence sessions, 26 baselines, one continuous update, zero later envelope comparisons, and zero trusted target outcomes, so it cannot produce a real validation score yet.
