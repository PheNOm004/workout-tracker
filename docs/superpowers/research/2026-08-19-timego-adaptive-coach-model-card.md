...
Each backtest holds out a completed session and uses only earlier sessions, ordered by session end, session ID, then set ID. Ordinary performance is evaluated against same-task historical envelopes and the continuous learner; binary target calibration is evaluated only where a genuine target outcome exists. Promotion requires enough real temporal boundaries, reviewed candidate metadata, calibrated uncertainty where binary labels exist, and counterexample review. The current read-only export has three closed weighted-evidence sessions, 26 neutral exercise/basis baselines, one later continuous update, and zero later same-exercise envelope comparisons. It cannot produce a validation score, so abstention remains correct.
